...
export const awardPoints = createAsyncThunk(
  ...
  async ({ activityType, metadata }, { rejectWithValue }) => {
    try {
      const response = await API.post('/api/points/award', { activityType, metadata });
      return response.data.data;
    } catch (error) {
...
export const fetchActivityHistory = createAsyncThunk(
  ...
  async ({ page = 1, limit = 20 }, { rejectWithValue }) => {
    try {
      const response = await API.get(`/api/points/activities?page=${page}&limit=${limit}`);
      return response.data.data;
    } catch (error) {
...
export const fetchLeaderboard = createAsyncThunk(
  ...
  async ({ limit = 50, period = 'all' }, { rejectWithValue }) => {
    try {
      const response = await API.get(`/api/points/leaderboard?limit=${limit}&period=${period}`);
      return response.data.data;
    } catch (error) {
...
export const recordSocialFollow = createAsyncThunk(
  ...
  async ({ platform }, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Failed to record follow');
    }
  }
);
